...
| `TEST_DATABASE_URL`                 | No       | Test database for `npm run test:e2e`. Defaults to an in-memory PGlite Postgres; e2e never uses `DATABASE_URL`. The opt-in push suite needs a real server here. |
