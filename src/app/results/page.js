...
export default function Results() {
  ...
  if (loading) {
    return <div className="container text-center mt-5">Calculating results...</div>;
  }
  ...
  if (!questionPaper || !userAnswers) {
    return (
      <div className="container text-center mt-5">
        ...
        <button className="btn btn-primary" onClick={() => router.push('/test')}>
          Take a Test
        </button>
      </div>
    );
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="alert alert-info text-center">
        <h2>Your Score: {score} / {questionPaper.questions.length}</h2>
      </div>
        ...
        {questionPaper.questions.map((q, index) => (
          <div key={index} className="card mb-3">
            ...
            <div className="card-body">
              <p><strong>Your Answer:</strong> <span className={userAnswers[index] === q.answer ? 'text-success' : 'text-danger'}>{userAnswers[index] || 'Not Answered'}</span></p>
              {userAnswers[index] !== q.answer && (
                <p><strong>Correct Answer:</strong> <span className="text-success">{q.answer}</span></p>
              )}
            </div>
          </div>
        ))}
