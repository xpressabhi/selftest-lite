...
export function setSessionCookie(cookies, rawSessionToken, expiresAt) {
	if (!rawSessionToken || !expiresAt || typeof cookies?.set !== 'function') {
		return;
	}
	cookies.set(SESSION_COOKIE_NAME, rawSessionToken, getSessionCookieOptions(expiresAt));
}
...
export function getRawSessionTokenFromRequest(cookies) {
...
export async function getAuthenticatedUser(cookies) {
	const session = await getSessionFromRequest(cookies, { refresh: false });
...
/**
 * Resolves the client-supplied identity header into a stable anonymous id.
 * Falls back to the legacy ip+ua derived key so older clients keep working.
 */
export function getClientIdFromRequest(request) {
	const headerValue = request?.headers?.get?.('x-client-id');
	return normalizeClientId(headerValue) || null;
}
