<script>
	...
	const postJsonLd = $derived(
		jsonLdScript({
			...
			mainEntityOfPage: `${SITE_ORIGIN}${localizedPath(`/blog/${slug}`, $activeLanguage)}`,
	...
	const formattedDate = $derived(formatBlogDate(post.date, $activeLanguage));
</script>

<SeoHead
	path={localizedPath(`/blog/${post.slug}`, $activeLanguage)}
	lang={$activeLanguage}
	type="article"
	title={`${$t(post.titleKey)} | selftest.in`}
	description={$t(post.excerptKey)}
/>
<svelte:head>
	{@html postJsonLd}
					...
					{#each related as post (post.slug)}
						<article class="post-related-card">
							<h3 class="post-related-card-title">
								<a class="post-link" href={localizedPath(`/blog/${post.slug}`, $activeLanguage)}>{$t(post.titleKey)}</a>
							</h3>
