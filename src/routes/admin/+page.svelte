<script>
	...
	async function logout() {
		...
		authed = false; stats = null; featureUsage = null;
	}
	...
	<div class="mx-auto admin-wrap">
		<div class="d-flex align-items-center justify-content-between gap-2 mb-3">
			...
			{#if authed}
				<div class="d-flex gap-2">
					<button class="btn btn-sm btn-outline-secondary" type="button" onclick={() => void loadAll()} disabled={checking}>
						{$t('adminRefresh')}
					</button>
		...
		{:else if !authed}
			<form class="admin-login bg-body border rounded-3 p-4 shadow-sm" onsubmit={(event) => { event.preventDefault(); void login(); }}>
				...
				<label class="form-label"><span class="fw-semibold">{$t('adminUsername')}</span>
					<input class="form-control mt-1" bind:value={username} autocomplete="username" /></label>
				<label class="form-label mt-3"><span class="fw-semibold">{$t('adminPassword')}</span>
					<input class="form-control mt-1" type="password" bind:value={password} autocomplete="current-password" /></label>
		...
		{:else if stats}
			...
			{#if activeTab === 'overview' && stats.overview}
				...
				<div class="overview-grid mb-4">
					...
					<div class="overview-card">
						...
						<div class="overview-recent">+{formatNumber(o.tables.ai_test_attempts.recent)} · avg {o.tables.ai_test_attempts.avg_score}%</div>
					</div>
					<div class="overview-card">
					...
					<div class="overview-card">
						...
						<div class="overview-recent">+{formatNumber(o.tables.api_request_events.recent)} · {formatNumber(o.tables.api_request_events.errors)} errors</div>
					</div>
					<div class="overview-card">
						<div class="overview-label">feature events</div>
						<div class="overview-total">{formatNumber(o.tables.feature_events.total)}</div>
						<div class="overview-recent">+{formatNumber(o.tables.feature_events.recent)} · {formatNumber(o.tables.feature_events.sessions)} sessions</div>
					</div>
				</div>
								...
								<div class="bg-body border rounded-3 p-3 h-100">
									...
									{#each o.testBreakdown.byMode as item (item.test_mode)}
										<div class="d-flex justify-content-between align-items-center border-bottom py-2">
											<span>{item.test_mode || '-'}</span><strong>{item.count}</strong>
										</div>
									{/each}
								</div>
							</section>
							...
							<section class="col-md-4">
								<div class="bg-body border rounded-3 p-3 h-100">
									...
									{#each o.testBreakdown.byDifficulty as item (item.difficulty)}
										<div class="d-flex justify-content-between align-items-center border-bottom py-2">
											<span>{item.difficulty || '-'}</span><strong>{item.count}</strong>
										</div>
									{/each}
								</div>
							</section>
						...
						<div class="hour-chart">
							{#each stats.hourly as item (item.bucket)}
								<div class="hour-bar" title={`${hourLabel(item.bucket)} — ${item.requests}`}>
									<div class="hour-bar-fill" style={`height: ${Math.max((item.requests / maxRequests) * 100, 3)}%`}></div>
						...
						<div class="bg-body border rounded-3 p-3">
							...
							<div class="table-responsive">
								<table class="admin-table">
									<thead><tr><th>{$t('adminColumnRoute')}</th><th>{$t('adminColumnRequests')}</th><th>{$t('adminErrors')}</th><th>{$t('adminAvgDuration')}</th></tr></thead>
									<tbody>
										{#each stats.byRoute || [] as item (item.route)}
											<tr><td class="mono">{item.route}</td><td>{formatNumber(item.requests)}</td><td>{formatNumber(item.errors)}</td><td>{item.avg_duration_ms} ms</td></tr>
										{/each}
									</tbody>
								</table>
							</div>
					...
					<section class="col-lg-6">
						<div class="bg-body border rounded-3 p-3">
							...
							{#each stats.byStatus || [] as item (item.status_code)}
								<div class="d-flex justify-content-between align-items-center border-bottom py-2">
									<span>{item.status_code ?? '-'}</span><strong>{formatNumber(item.requests)}</strong>
								</div>
							{/each}
							{#if !(stats.byStatus?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
						</div>
					</section>
				</div>
			{/if}
					...
					<section class="col-lg-6">
						<div class="bg-body border rounded-3 p-3">
							...
							{#each stats.byCountry || [] as item (item.country)}
								<div class="d-flex justify-content-between align-items-center border-bottom py-2">
									<span>{item.country}</span><strong>{formatNumber(item.requests)}</strong>
								</div>
							{/each}
							{#if !(stats.byCountry?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
						</div>
					</section>
					<section class="col-lg-6">
						<div class="bg-body border rounded-3 p-3">
							...
							{/each}
							{#if !(stats.topAgents?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
						</div>
					</section>
				</div>
			{/if}
			...
			{#if activeTab === 'recent'}
				<div class="bg-body border rounded-3 p-3 mb-4">
					...
					<div class="table-responsive">
						<table class="admin-table">
							<thead><tr><th>{$t('adminColumnTime')}</th><th>{$t('adminColumnRoute')}</th><th>{$t('adminColumnAction')}</th><th>{$t('adminColumnStatus')}</th><th>{$t('adminColumnDuration')}</th><th>{$t('adminColumnCountry')}</th><th>{$t('adminColumnCity')}</th><th>{$t('adminColumnClient')}</th></tr></thead>
							<tbody>
					...
					{#if !(stats.recent?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
				</div>
			{/if}
			...
			{#if activeTab === 'features'}
				<div class="bg-body border rounded-3 p-3 mb-4">
					<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
						...
						<button class="btn btn-sm btn-outline-secondary" type="button" onclick={() => void loadFeatureUsage()}>{$t('adminRefresh')}</button>
					</div>
					{#if featureUsage}
						<div class="stat-cards mb-4">
							<div class="bg-body border rounded-3 p-3"><strong>{formatNumber(featureUsage.totals?.total)}</strong><span>{$t('adminFeatureEvents')}</span></div>
							<div class="bg-body border rounded-3 p-3"><strong>{formatNumber(featureUsage.totals?.sessions)}</strong><span>{$t('adminFeatureSessions')}</span></div>
							<div class="bg-body border rounded-3 p-3"><strong>{featureUsage.totals?.events_per_session || 0}</strong><span>{$t('adminFeaturePerSession')}</span></div>
							<div class="bg-body border rounded-3 p-3"><strong>{formatNumber(featureUsage.byEvent?.length)}</strong><span>{$t('adminFeatureDistinct')}</span></div>
						</div>
						<div class="row g-3 mb-4">
							<section class="col-lg-6">
								<div class="bg-body border rounded-3 p-3 h-100">
									...
									{#each featureUsage.byEvent || [] as item (item.event)}
										...
										<div class="mb-2">
											<div class="d-flex justify-content-between gap-2 small mb-1">
												...
												<span class="text-nowrap"><strong>{formatNumber(item.count)}</strong> <span class="text-muted">({item.pct ?? 0}%)</span></span>
											</div>
											<div class="feature-bar"><div class="feature-bar-fill" style={`width: ${Math.max((item.count / maxCount) * 100, 2)}%`}></div></div>
										</div>
									{/each}
									{#if !(featureUsage.byEvent?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
								</div>
							</section>
							<section class="col-lg-6">
								<div class="bg-body border rounded-3 p-3 h-100">
										...
										<div class="hour-chart">
											{#each featureUsage.trend as item (item.day)}
												<div class="hour-bar" title={`${dayLabel(item.day)} — ${item.events} (${item.sessions} sessions)`}>
													<div class="hour-bar-fill" style={`height: ${Math.max((item.events / maxTrend) * 100, 3)}%`}></div>
													<span class="hour-bar-label">{item.sessions}</span>
												</div>
											{/each}
										</div>
									{:else}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
								</div>
							</section>
						</div>
						<div class="row g-3">
							<section class="col-lg-6">
								<div class="bg-body border rounded-3 p-3 h-100">
									...
									{#each featureUsage.byPage || [] as item (item.page)}
										<div class="d-flex justify-content-between align-items-center border-bottom py-2">
											<span class="mono text-truncate pe-2">{item.page}</span><strong>{formatNumber(item.events)}</strong>
										</div>
									{/each}
									{#if !(featureUsage.byPage?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
								</div>
							</section>
							<section class="col-lg-6">
								<div class="bg-body border rounded-3 p-3 h-100">
									...
									<div class="table-responsive">
										<table class="admin-table">
											<thead><tr><th>{$t('adminFeatureMode')}</th><th>{$t('adminFeatureDifficulty')}</th><th>{$t('adminFeatureLanguage')}</th><th>{$t('adminFeatureCount')}</th></tr></thead>
											<tbody>
												{#each featureUsage.generateBreakdown || [] as item (`${item.mode}-${item.difficulty}-${item.language}`)}
													<tr><td class="mono">{item.mode}</td><td class="mono">{item.difficulty}</td><td class="mono">{item.language}</td><td>{formatNumber(item.count)}</td></tr>
												{/each}
											</tbody>
										</table>
									</div>
									{#if !(featureUsage.generateBreakdown?.length)}<p class="text-muted small mb-0">{$t('adminEmpty')}</p>{/if}
								</div>
							</section>
						</div>
					{:else}
						<div class="py-4 text-center"><div class="thinking-dots" role="status"><span></span><span></span><span></span></div></div>
					{/if}
				</div>
			{/if}
		{/if}
...
<style>
	...
	.stat-cards > div, .overview-card {
		display: flex; min-height: 72px; align-items: center; flex-direction: column; justify-content: center; text-align: center;
	}
	.stat-cards strong, .overview-total { font-size: 1.25rem; font-weight: 700; }
	.stat-cards span { color: var(--text-muted); font-size: 0.75rem; }

	.overview-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; }
	.overview-card { background: var(--surface); border: 1px solid var(--line); border-radius: 0.75rem; padding: 12px 8px; min-height: 76px; }
	.overview-label { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.03em; color: var(--text-muted); }
	.overview-recent { color: var(--text-muted); font-size: 0.65rem; margin-top: 2px; }

	.hour-chart { display: flex; height: 140px; align-items: flex-end; gap: 3px; }
	.hour-bar { display: flex; flex: 1; min-width: 0; height: 100%; align-items: center; flex-direction: column; justify-content: flex-end; gap: 2px; }
	.hour-bar-fill { width: 100%; min-height: 3px; border-radius: 4px 4px 0 0; background: var(--color-brand-600); }
	.hour-bar-label { color: var(--text-muted); font-size: 0.65rem; }

	.feature-bar { height: 8px; overflow: hidden; border-radius: 4px; background: color-mix(in srgb, var(--line) 60%, transparent); }
	.feature-bar-fill { height: 100%; border-radius: 4px; background: var(--color-brand-600); }

	.admin-table { width: 100%; font-size: 0.85rem; border-collapse: collapse; }
	.admin-table th { padding: 8px 10px; text-align: left; border-bottom: 1px solid var(--line); color: var(--text-muted); font-size: 0.75rem; white-space: nowrap; }
	.admin-table td { padding: 7px 10px; border-bottom: 1px solid var(--line); vertical-align: top; white-space: nowrap; }
	.admin-table tr:last-child td { border-bottom: 0; }
	.mono { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.78rem; }
	.min-w-0 { min-width: 0; }

	@media (max-width: 767.98px) {
		.stat-cards, .overview-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
