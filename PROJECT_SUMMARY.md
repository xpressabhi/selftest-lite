...
### 8. Account-free operation
- The app does not implement user accounts, authentication, cloud state sync, or user-attempt persistence.
- Quiz history, bookmarks, preferences, and attempts remain local-first in browser storage.
...
Primary storage uses Neon PostgreSQL for generated tests and operational telemetry. Authentication and user-sync tables are intentionally excluded.
...
- User/authentication, session, cloud-state, and user-attempt tables are intentionally not part of the schema.
